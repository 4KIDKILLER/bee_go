...
func (fileDao *FileDao) Insert(parentId, fileId, fileOriginalName, fileExt, filePath, tags, cover1, cover2, cover3, remark string, userId, fileType int, fileSize decimal.Decimal) (result sql.Result, err error) {
...
func (fileDao *FileDao) QueryFilesByUserId(parentId string, userId, page, pageSize int) (result []*model.BeeFile, err error) {
	...
	err = fileDao.mysql.Select(&result, "SELECT `parent_id`,`file_id`,`user_id`,`file_original_name`,`file_ext`,`file_size`,`file_type`,`tags`,`cover_1`,`cover_2`,`cover_3`,`remark`,`create_time`,`update_time` FROM `bee_file` WHERE `user_id`=? AND `parent_id`=? AND `status`=1 ORDER BY `create_time` DESC LIMIT ?, ?", userId, parentId, page, pageSize)
