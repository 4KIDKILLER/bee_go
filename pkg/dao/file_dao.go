...
func (fileDao *FileDao) UpdateStatusByFileIdInIds(fileIds []string, status, userId int) (result sql.Result, err error) {
	result, err = fileDao.mysql.Exec("UPDATE bee_file SET status=? WHERE file_id IN (?)", status, fileIds)
	return
}

func (fileDao *FileDao) SelectRecursionFilesByFolderId(fileId, userId int) (result []*model.BeeFile, err error) {
