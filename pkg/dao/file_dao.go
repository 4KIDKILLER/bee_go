...
func (fileDao *FileDao) UpdateThumbPathByFileId(thumbPath, fileId string) (result sql.Result, err error) {
	result, err = fileDao.mysql.Exec("UPDATE `bee_file` SET `file_thumb_path`=? WHERE `file_id`=?", thumbPath, fileId)
	return
}
